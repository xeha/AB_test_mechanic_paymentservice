...
def download_data(public_key, local_path = None, separ = ';'):
...
def collect_summary_df(groups_df, groups_add_df, active_studs_df, checks_df):
    ...
    summary_df = pd.merge(checks_df, experiment_users, how='right', left_on='student_id', right_on='id').drop('student_id', axis=1)
...
def consolidation_df(experiment_df):
    ...
    paying_users_df = (
    experiment_df_cr.query('converted==1').reset_index().groupby('grp').agg(paying_users=('id', 'nunique')).reset_index()
)
